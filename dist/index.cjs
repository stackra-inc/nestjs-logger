...
function Injectable(options) {
  return (target) => {
    metadata.defineMetadata(INJECTABLE_WATERMARK, true, target);
    metadata.defineMetadata(SCOPE_OPTIONS_METADATA, options, target);
  };
}
...
var DiscoveryService = class {
    ...
    const decorator = ((opts) => {
      ...
      return ((target, propertyKey, descriptor) => {
        if (descriptor && propertyKey !== void 0) {
          metadata.defineMetadata(metadataKey, value, descriptor.value);
          return descriptor;
        }
        DiscoverableMetaHostCollection.addClassMetaHostLink(target, metadataKey);
        metadata.defineMetadata(metadataKey, value, target);
        return target;
      });
    });
  ...
  getMetadataByDecorator(decorator, wrapper, methodKey) {
    if (methodKey) {
      ...
      return metadata.getMetadata(decorator.KEY, method);
    }
    const classRef = wrapper.instance?.constructor ?? wrapper.metatype;
    if (!classRef) {
      return void 0;
    }
    return metadata.getMetadata(decorator.KEY, classRef);
  }
  /**
...
function Inject(token) {
  ...
  return (target, key, index) => {
    ...
    if (!resolvedToken && !hasExplicitToken) {
      if (key !== void 0) {
        resolvedToken = metadata.getMetadata("design:type", target, key);
      } else if (index !== void 0) {
        const paramTypes = metadata.getMetadata(PARAMTYPES_METADATA, target) ?? [];
...
function Module(metadata$1) {
  const invalidKeys = Object.keys(metadata$1).filter((key) => !VALID_MODULE_KEYS.has(key));
  ...
  return (target) => {
    for (const property in metadata$1) {
      if (!Object.prototype.hasOwnProperty.call(metadata$1, property)) continue;
      const metadataKey = PROPERTY_TO_METADATA_KEY[property];
      metadata.defineMetadata(metadataKey, metadata$1[property], target);
    }
  };
}
function Global() {
  return (target) => {
    metadata.defineMetadata(GLOBAL_MODULE_METADATA, true, target);
  };
}
var Reflector = class {
  /**
  ...
  get(metadataKey, target) {
    return metadata.getMetadata(metadataKey, target);
  }
  /**
  ...
  getAllAndOverride(metadataKey, targets) {
    for (const target of targets) {
      const result = metadata.getMetadata(metadataKey, target);
      if (result !== void 0) {
        return result;
      }
    }
  ...
  getAllAndMerge(metadataKey, targets) {
    const result = [];
    for (const target of targets) {
      const value = metadata.getMetadata(metadataKey, target);
...
function Reporter(options) {
  return (target) => {
    Injectable()(target);
    metadata.defineMetadata(REPORTER_METADATA, options, target);
  };
}
...
exports.ConsoleReporter = class ConsoleReporter {
  /**
...
exports.ConsoleReporter = __decorateClass2([
...
var LoggerService = class _LoggerService {
  constructor(configOrContext) {
    ...
    if (typeof configOrContext === "string" || configOrContext === void 0) {
      ...
      this._reporters = configOrContext.reporters ?? [new exports.ConsoleReporter()];
  ...
  static get _fallbackLogger() {
    if (!_LoggerService._fallbackLoggerInstance) {
      _LoggerService._fallbackLoggerInstance = new _LoggerService({
        reporters: [new exports.ConsoleReporter({ level: LogLevel.Warn })]
      });
    }
...
exports.SilentReporter = class SilentReporter {
  constructor() {
    this.name = "silent";
    this._level = LogLevel.Debug;
  }
...
exports.SilentReporter = __decorateClass2([
  Reporter({ name: "silent" })
], exports.SilentReporter);
exports.LoggerManager = class LoggerManager extends MultipleInstanceManager {
  /**
  ...
  createDriver(driver, config) {
    ...
    if (!channelConfig.reporters || channelConfig.reporters.length === 0) {
      if (driver === "silent") {
        return { ...channelConfig, reporters: [new exports.SilentReporter()] };
      }
      return { ...channelConfig, reporters: [new exports.ConsoleReporter()] };
    }
...
exports.LoggerManager = __decorateClass2([
  Injectable(),
...
var ReporterLoader = class {
  ...
  loadReporters() {
    ...
    for (const wrapper of providers) {
      ...
      const reporterOptions = metadata.getMetadata(
        REPORTER_METADATA,
        constructor
      );
  ...
  static forRoot(config) {
    ...
    return {
      ...
      providers: [
        ...
        { provide: exports.LoggerManager, useClass: exports.LoggerManager },
        { provide: LOGGER_MANAGER, useExisting: exports.LoggerManager },
        ReporterLoader,
        {
      ...
      exports: [exports.LoggerManager, LOGGER_MANAGER, LOGGER_CONFIG, ...channelTokens]
    };
  }
};
exports.LoggerModule = __decorateClass2([
  Global(),
  Module({})
], exports.LoggerModule);
var Logger = LoggerService;
exports.StorageReporter = class StorageReporter {
  /**
...
exports.StorageReporter = __decorateClass2([
  Reporter({ name: "storage" })
], exports.StorageReporter);
...
exports.InjectLoggerManager = InjectLoggerManager;
